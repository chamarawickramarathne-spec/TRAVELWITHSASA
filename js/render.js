        ...
        <div class="animate-in fade-in slide-in-from-bottom-5 duration-700">
            <header class="relative h-[60vh] min-h-[400px] w-full">
                ...
                <div class="absolute inset-0 flex items-center justify-center p-6 text-center">
                    <div class="max-w-4xl">
                        <span class="inline-block px-4 py-1 rounded-full bg-primary text-white text-sm font-bold uppercase tracking-widest mb-4">${data.category}</span>
                        <h1 class="text-4xl md:text-6xl font-black text-white leading-tight mb-4 drop-shadow-2xl">${data.title}</h1>
                        <div class="flex items-center justify-center gap-4 text-white/80 text-sm md:text-base">
                            <span class="flex items-center gap-1"><i data-lucide="map-pin" class="w-4 h-4"></i> ${data.location}</span>
                ...
                <div class="grid grid-cols-1 lg:grid-cols-3 gap-12">
                    <article class="lg:col-span-2 bg-black/40 backdrop-blur-xl border border-white/10 p-8 md:p-12 rounded-3xl shadow-2xl">
                        ...
                        <div class="mt-12 pt-12 border-t border-white/10">
                            <h3 class="text-lg font-bold text-white mb-4">${lang === 'si' ? 'ටැග්' : 'Tags'}</h3>
                    ...
                    <aside class="space-y-8">
                        ...
                        <div class="bg-primary/10 border border-primary/20 p-8 rounded-3xl relative overflow-hidden group">
                           <div class="relative z-10">
                                <h3 class="text-xl font-bold text-white mb-4">${lang === 'si' ? 'සංචාරය සැලසුම් කරන්න' : 'Plan Your Journey'}</h3>
                                <p class="text-gray-300 text-sm mb-6">${lang === 'si' ? 'සැබෑ ලංකාව අත්විඳින්න අප උදව් කරන්නම්.' : `Want to visit ${data.location}? Let us help you plan an authentic adventure.`}</p>
                                <button class="w-full py-3 px-6 bg-primary hover:bg-primary-dark text-white font-bold rounded-xl transition-all shadow-lg shadow-primary/20">${lang === 'si' ? 'චාරිකාව සැලසුම් කරන්න' : 'Get Custom Itinerary'}</button>
                           </div>
